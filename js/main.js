...
const account = `    <div class="account-setting">
<div>
  <div class="account-s">
    <div class="pic"><img src="imgs/kaska.jpg" alt="" /></div>
    <div class="info">
      <p>Mohamed Saleh</p>
...
<div class="notfi-content">
  <div class="notfi container">
    <div class="notif">
      <div class="user-img"><img src="imgs/kaska.jpg" alt="" /></div>
      <div class="text">
        <h1>Medo says Hi</h1>
        <span>1 day ago</span>
      </div>
  ...
  <div class="center">
    ...
    <div class="left">
      <div class="cont">
        <div class="vid">
          <div>
            <video src="User/Videos/Parker Dance2.mp4"></video>
          </div>
...
const uploadPreviewVidCont = document.querySelector('.UploadCont .createVideo .center .vid')
const uploadPreviewVid = document.querySelector('.UploadCont .createVideo .center video')
const timeLineContaienrUpload = document.querySelector('.UploadCont .createVideo .center .vid .timeline-container .timeline-holder')
const goingUplaod = document.querySelector('.UploadCont .createVideo .center .vid #going')

let wasPausedUpload
let scrrubingUpload = false
timeLineContaienrUpload.addEventListener('click' , progress)
timeLineContaienrUpload.addEventListener('mousemove', previewTimeLine)
timeLineContaienrUpload.addEventListener('mousedown' , () => {
  scrrubingUpload = true
  wasPausedUpload = uploadPreviewVid.paused
  uploadPreviewVid.pause()
})
document.addEventListener('mouseup' , (e) => {
  if(scrrubingUpload) {
    e.preventDefault();
    scrrubingUpload = false
    timeLineContaienrUpload.classList.remove('scrubbing')
    if(!wasPausedUpload) uploadPreviewVid.play()
  }
})
timeLineContaienrUpload.addEventListener('mousemove' , (e) => {
  if(scrrubingUpload) {
    timeLineContaienrUpload.classList.add('scrubbing')
    e.preventDefault();
    progress(e)
    previewTimeLine(e)
  }
...
function progress(e) {
  const rect = timeLineContaienrUpload.getBoundingClientRect();
  const percent =
    Math.min(Math.max(0, e.x - rect.x), rect.width) / rect.width;
    uploadPreviewVid.currentTime = percent * uploadPreviewVid.duration;
  timeLineContaienrUpload.style.setProperty("--progress-position", percent);
  }
function previewTimeLine(e) {
  const rect = timeLineContaienrUpload.getBoundingClientRect();
  const percent = Math.min(Math.max(0, e.x - rect.x), rect.width) / rect.width;
  const cuurenttime = percent * uploadPreviewVid.duration;
  let percentMax =
    (rect.width - goingUplaod.clientWidth / 2) / rect.width;
  let percentLow =
    1 - (rect.width - goingUplaod.clientWidth / 2) / rect.width;
  ...
  timeLineContaienrUpload.style.setProperty("--preview-position", percent);
  timeLineContaienrUpload.style.setProperty("--preview-position-time", percentTwo);
...
function playPause() {
  if (uploadPreviewVid.paused == true) {
    uploadPreviewVid.play();
  } else {
    uploadPreviewVid.pause();
  }
}
